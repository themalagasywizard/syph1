...
const getNearbyClients = (): NearbyClients => ({
  "Acme Corp": {
    distance: "0.5 miles",
    relationship: {
      yearsAsClient: 3,
...
export default function NearbyClientsWidget({ location: _ }: NearbyClientsWidgetProps) {
  ...
  const nearbyClients = getNearbyClients();
    ...
    <>
      <Card>
        ...
        <div className="mt-4 space-y-3">
          {Object.entries(nearbyClients).map(([clientName, data], index) => (
            <button
              key={index}
      ...
      {selectedClient && (
        <ClientDetailModal
          ...
          clientData={nearbyClients[selectedClient]}
        />
      )}
    </>
  );
